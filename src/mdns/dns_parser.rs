...
use super::{Error, Result, ServiceInfo};
...
pub struct DnsOutgoing {
    ...
    pub answers: Vec<(Box<dyn DnsRecordExt>, u64)>,
    pub authorities: Vec<DnsPointer>,
...
impl DnsOutgoing {
    ...
    //    o  All address records (type "A" and "AAAA") named in the SRV rdata.
    pub fn add_additional_answer(&mut self, answer: DnsRecordBox) {
    ...
    pub fn add_answer(&mut self, msg: &DnsIncoming, answer: Box<dyn DnsRecordExt>) -> bool {
    ...
    pub fn add_answer_at_time(&mut self, answer: Box<dyn DnsRecordExt>, now: u64) -> bool {
    ...
    fn read_others(&mut self) -> Result<()> {
        ...
        for _ in 0..n {
            ...
            let rec: Option<DnsRecordBox> = match ty {
                ...
                _ => {
                    tracing::error!("Unknown DNS record type [{ty}]");
                    self.offset += length;
                    None
                }
            };
